human_footprint_rasters <- list.files("data/HumanFootprint/Maps/", full.names = TRUE) %>%
                                    grep(".tif$", .,  value = TRUE) %>%
                                    magrittr::extract(c(5, 7, 1, 2, 3, 4, 11, 12, 13, 14, 15, 16, 18)) %>%
                                    magrittr::set_names(c("hfp_1993", "hfp_2009",
                                                          "built_1993", "built_2009",
                                                          "croplands_1993", "croplands_2009",
                                                          "navwater_1993", "navwater_2009",
                                                          "pasture_1993", "pasture_2009",
                                                          "popdens_1993", "popdens_2009",
                                                          "roads"))

connectivity_rasters <- human_footprint_rasters[c("navwater_2009",
                                                  "popdens_2009",
                                                  "roads")]


## Divide by "pressure" scores assigned in paper: https://www.nature.com/articles/sdata201667
scaling <- c("hfp_1993" = 1, "hfp_2009" = 1,
             "built_1993" = 10, "built_2009" = 10,
             "croplands_1993" = 7, "croplands_2009" = 7,
             "navwater_1993" = 4, "navwater_2009" = 4,
             "pasture_1993" = 4, "pasture_2009" = 4,
             "popdens_1993" = 1, "popdens_2009" = 1,
             "roads" = 8)

connectivity_scaling <- scaling[c("navwater_2009",
                                  "popdens_2009",
                                  "roads")]

actual_years <- c("hfp_1993" = 1993, "hfp_2009" = 2009,
                  "built_1993" = 1994, "built_2009" = 2009,
                  "croplands_1993" = 1992, "croplands_2009" = 2005,
                  "navwater_1993" = 1994, "navwater_2009" = 2009,
                  "pasture_1993" = 1993, "pasture_2009" = 2009,
                  "popdens_1993" = 2005, "popdens_2009" = 2020,
                  "roads" = 2005)

forest_gain_files <- list.files(file_in("data/forest_cover/gain/"), full.names = TRUE) 

problematic_territory_names <- c("Guernsey (Channel Islands)" = "Guernsey",
...
binaryish <- c("world_language_as_official", 
               "minority_education_policy",
...
world_langs <- list("Portuguese" = c("por"), 
                    "Arabic" = c("aao", "abh", "abv", "acm", "acq", "acw", "acx", "acy", "adf", "aeb", "aec", "afb", "ajp", "apc", "apd", "arb", "arq", "ars", "ary", "arz", "auz", "avl", "ayh", "ayl", "ayn", "ayp", "bbz", "pga", "shu", "ssh"), 
                    "Spanish" = c("spa"), 
                    "English" = c("eng"), 
                    "French" = c("fra"), 
                    "Russian" = c("rus"),
                    "Bahasa" = c("bjn", "btj", "bve", "bvu", "coa", "dup", "hji", "ind", "jak", "jax", "kvb", "kvr", "kxd", "lce", "lcf", "liw", "max", "meo", "mfa", "mfb", "min", "mly", "mqg", "msi", "mui", "orn", "ors", "pel", "pse", "tmw", "urk", "vkk", "vkt", "xmm", "zlm", "zmi", "zsm"), 
                    "Mandarin" = c("cdo", "cjy", "cmn", "cpx", "czh", "czo", "gan", "hak", "hsn", "lzh", "mnp", "nan", "wuu", "yue"),
                    "Hindustani" = c("hin", "urd"))
...
plan <- drake_plan(
  ...
  # report = rmarkdown::render(knitr_in("preliminary_report.Rmd"), 
  #              output_file = file_out("language_endangerment_preliminary_report.html")),
  
  # lang_polys_orig = sf::st_read(file_in("data/lang/langa.shp")),
  # 
  # lang_pts = sf::st_read(file_in("data/lang_pt/lang_all_pt.shp")),
  # 
  # lang_bnd = sf::st_read(file_in("data/lang_bnd/DCW_countries.shp")),
  # 
  # wlms_dat = foreign::read.dbf("data/lang_dbf/eth_wlms.dbf"),
  ...
  extinct_e16 = lang_pts_e16 %>%
    dplyr::filter(LMP_CLASS == "X"),
  
  extinct_e16_data = extinct_e16 %>%
    dplyr::as_tibble() %>%
    dplyr::select(LANG_ISO, NAM_LABEL, LMP_POP1) %>%
    readr::write_csv(file_out("data/extinct_e16_data.csv")),
  
  extinct_e16_map = mapview::mapview(extinct_e16, col.regions = "white", 
                                     zcol = c("LANG_ISO"),
                                     legend = FALSE),
  
  world_langs_macro = purrr::imap_dfr(world_langs,
                                      ~dplyr::tibble(world_language = .y,
                                                     LANG_ISO_micro = .x)) %>%
    dplyr::left_join(lang_polys %>%
                       dplyr::as_tibble() %>%
                       dplyr::select(LANG_ISO_micro,
                                     LANG_ISO) %>%
                       dplyr::group_by(LANG_ISO_micro) %>%
                       dplyr::summarise(LANG_ISO = LANG_ISO[1])) %>%
    dplyr::pull(LANG_ISO) %>%
    unique() %>%
    na.omit() %>%
    as.vector(),
  
  # extinct_e16_map_html = mapview::mapshot(extinct_e16_map, file_out("interactive/extinct_e16.html"),
  ...
  lang_widespread_ids_labeled = lang_widespread_ids %>%
    as.list() %>%
    magrittr::set_names(attr(lang_widespread_ids, "region.id")),
  
  lang_polys_widespread = base_map_states[unlist(lang_widespread_ids_labeled), ] %>%
    dplyr::select(geometry) %>%
    dplyr::bind_cols(lang_pts_widespread[as.numeric(names(unlist(lang_widespread_ids_labeled))), ] %>%
                       dplyr::as_tibble() %>%
                       dplyr::select(-geometry)),

  #poly_cols = intersect(colnames(lang_polys_orig), colnames(lang_polys_voronoi)),
  
  new_pop = readr::read_csv(file_in("data/hedvig_output/POP_manual.csv")),
    
  lang_polys = rbind(lang_polys_orig %>%
                       dplyr::select(!!poly_cols), 
                     lang_polys_voronoi %>%
                       dplyr::select(!!poly_cols), 
                     lang_polys_widespread %>%
                       dplyr::select(!!poly_cols)) %>%
    add_macrolanguages(macro_languages) %>%
    dplyr::left_join(new_pop) %>%
    dplyr::mutate(LMP_POP1 = ifelse(is.na(`POP new`), LMP_POP1, `POP new`)) %>%
    dplyr::select(-`POP new`) %>%
    ## fix 3 of 5 polygon errors
    dplyr::filter(!ID %in% c("CYM-ARG", "GLE-CAN", "vec-BRA", "vec-MEX", "vec-HRV")) %>%
    ## fix fiji problem (error 4 of 5)
    sf::st_wrap_dateline(options = "WRAPDATELINE=YES") %>%
    sf::st_cast("MULTIPOLYGON"),
  
  sign = readr::read_tsv("data/sign/list_sign_languages_glottolog_44.tsv"),
  
  lang_polys_sign = rbind(lang_polys_orig %>%
                       dplyr::select(!!poly_cols) %>%
                         mutate(range = "poly"), 
                     lang_polys_voronoi %>%
                       dplyr::select(!!poly_cols) %>%
                       mutate(range = "point_voronoi"), 
                     lang_polys_widespread %>%
                       dplyr::select(!!poly_cols) %>%
                       mutate(range = "point_state")) %>%
    add_macrolanguages(macro_languages) %>%
    dplyr::left_join(sign, by = c("LANG_ISO" = "ISO639P3code")) %>%
    dplyr::mutate(sign = !is.na(Glottocode)) %>%
    filter(sign) %>%
    left_join(glottolog_all %>% select(LANG_ISO, category, med)),
  ...
  base_map_states = rnaturalearth::ne_states(returnclass = "sf") %>%
    sf::st_make_valid(),
  ...
  base_map_unioned = base_map %>%
    sf::st_union() %>%
    smoothr::fill_holes(units::set_units(1000, km^2)) %>%
    sf::st_set_crs(base_map_crs),
  
  sign_lang_map = make_sign_map(lang_polys_sign, base_map_unioned),
  
  lang_ids = lang_polys %>%
    as.data.frame() %>%
    dplyr::select(ID:NAM_ANSI, LANG_ISO),
  
  lang_ids_ISO_csv = lang_ids_ISO %>%
    readr::write_csv(file_in("data/lang_ids_ISO.csv")),
  ...
  hua_language_div_2000 = hua_data_2000 %>%
    dplyr::mutate(language_diversity = ifelse(land_coverage == 0, NA, language_diversity)),
  
  hua_data_480 = read_rds(file_in("data/Hua_et_al_data_480.rds")),
  
  hua_language_div_480 = hua_data_480 %>%
    dplyr::mutate(language_diversity = ifelse(land_coverage == 0, NA, language_diversity)),
  
  hua_data_12600 = read_rds(file_in("data/Hua_et_al_data_12600.rds")),
  
  hua_language_div_12600 = hua_data_12600 %>%
    dplyr::mutate(language_diversity = ifelse(land_coverage == 0, NA, language_diversity)),
  
  hua_data_12600_language_diversity = raster::rasterFromXYZ(hua_language_div_12600[ , c("longitudes", 
                                                                                      "latitudes", 
                                                                                      "language_diversity")],
  ...
  hua_data_2000_language_diversity = raster::rasterFromXYZ(hua_language_div_2000[ , c("longitudes", 
                                                                                      "latitudes", 
                                                                                      "language_diversity")],
                                                           crs = sp::CRS("+proj=cea +datum=WGS84 +units=m")) %>%
  ...
  hua_data_480_language_diversity = raster::rasterFromXYZ(hua_language_div_480[ , c("longitudes", 
                                                                                      "latitudes", 
                                                                                      "language_diversity")],
                                                           crs = sp::CRS("+proj=cea +datum=WGS84 +units=m")) %>%
  ...
  climate_rast = raster::stack(list(mean_temp = raster::raster(file_in("data/climate/wc2.0_bio_2.5m_01.tif")),
  ...
  ts_projections_rast = raster::stack(raster::stack(file_in("data/projections/tempseasonality/wc2.1_2.5m_bioc_CNRM-ESM2-1_ssp370_2041-2060.tif"))[[4]],
  ...
  alt_rast = raster::raster(file_in("data/climate/alt.bil")) %>%
    raster::projectRaster(crs = sp::CRS(sf::st_crs(lang_polys_merged_cea)$proj4string)) %>%
    raster::writeRaster(file_out("data/climate/alt.grd"),
                        overwrite = TRUE),
  ...
  langs_num_polygons = purrr::map_int(lang_polys_unioned$geometry,
                                            ~length(.x)),
  ...
  lang_circle_intersections = lang_circles %>%
  ...
  bordering_languages = lang_polys_merged %>%
    dplyr::as_tibble() %>%
    dplyr::select(LANG_ISO) %>%
    dplyr::mutate(neighbours = purrr::map_int(nb_net, ~ sum(. != 0)),
                  neighour_list_indices = unclass(nb_net),
                  neighbour_list_ids = purrr::map(nb_net, ~ lang_polys_merged$LANG_ISO[.] %>% as.character),
                  neighbour_list_pops = purrr::map(nb_net, ~ lang_polys_merged$LMP_POP1[.]),
                  perimeter = language_perimeters_merged %>% as.numeric()),
  
  language_bordering_diversity = bordering_languages %>%
    dplyr::mutate(bordering_language_richness = neighbours %>% as.numeric(),
                  bordering_language_richness_per_km_perim = bordering_language_richness / perimeter,
                  bordering_language_sw = purrr::map_dbl(neighbour_list_pops,
                                                         ~ asbio::SW.index(.x)),
  ...
  IUCN_mammals_VU_higher = IUCN_mammals %>%
    dplyr::filter(code %in% c("VU", "EN", "CR", "EW", "EX"),
                  terrestial == "t") %>%
  ...
  IUCN_amphibians_VU_higher = IUCN_amphibians %>%
    dplyr::filter(category %in% c("VU", "EN", "CR", "EW", "EX")) %>%
    sf::st_transform(crs = sf::st_crs(IUCN_grid)),
  ...
  IUCN_all_polys_cea = dplyr::bind_rows(IUCN_mammals %>%
                                          dplyr::filter(code != "DD") %>%
                                          sf::st_transform(crs = sf::st_crs(IUCN_grid)), 
                                    IUCN_amphibians %>%
  ...
  IUCN_mammals_raster_high_res = target(fasterize::fasterize(IUCN_mammals_VU_higher, hua_data_12600_language_diversity,
                                             fun = "sum") %>%
                                          raster::writeRaster(file_out("data/IUCN/mammals/mammal_count_high_res.grd"), overwrite = TRUE),
    trigger = trigger(condition = fix_5_errors != 1, mode = "blacklist")),
  
  IUCN_mammals_raster_low_res = raster::resample(IUCN_mammals_raster_high_res, 
                                                 hua_data_480_language_diversity) %>%
    raster::writeRaster(file_out("data/IUCN/mammals/mammal_count_low_res.grd"), overwrite = TRUE),
  
  IUCN_amphibians_raster_high_res = target(fasterize::fasterize(IUCN_amphibians_VU_higher, hua_data_12600_language_diversity,
                                                      fun = "sum") %>%
                                             raster::writeRaster(file_out("data/IUCN/amphibians/mammal_count_high_res.grd"), overwrite = TRUE),
    trigger = trigger(condition = fix_5_errors != 1, mode = "blacklist")),
  
  IUCN_amphibians_raster_low_res = raster::resample(IUCN_amphibians_raster_high_res, 
                                                 hua_data_480_language_diversity) %>%
    raster::writeRaster(file_out("data/IUCN/amphibians/mammal_count_low_res.grd"), overwrite = TRUE),
  ...
  languages_by_grid = sf::st_intersects(new_language_grid,
                                         lang_polys_merged_cea %>%
                                           dplyr::select(LANG_ISO, LMP_POP1, AES)),
  
  IUCN_by_grid = target(sf::st_intersects(IUCN_grid,
                                   IUCN_polys_cea %>%
                                          dplyr::select(binomial)),
                        trigger = trigger(condition = fix_5_errors != 1,
                                          mode = "blacklist")),
  
  IUCN_all_by_grid = target(sf::st_intersects(IUCN_grid,
                                   IUCN_all_polys_cea %>%
                                     dplyr::select(binomial)),
                            trigger = trigger(condition = fix_5_errors != 1,
                                              mode = "blacklist")),
  
  diversity_raster_480 = make_diversity_raster_480(languages_by_grid,
                                                   #IUCN_by_grid,
                                                   #IUCN_all_by_grid,
                                                   hua_data_480_language_diversity,
                                                   lang_polys_merged_cea),
    ...
    # sf::st_transform(sp::proj4string(base_raster_high_res_cea)) %>%
    # fasterize::fasterize(base_raster_high_res_cea,
    #                      field = "LMP_POP1",
    #                      by = "LANG_ISO"),
    
  
  language_grid_based_data = get_grid_based_data(lang_polys_cea,
                                                 diversity_raster_480),
  
  language_grid_based_data_by_ISO = language_grid_based_data %>%
    dplyr::as_tibble() %>%
    dplyr::mutate(area = as.vector(language_areas) / (1000 * 1000)) %>%
    dplyr::group_by(LANG_ISO) %>%
    dplyr::mutate(area_prop = area / sum(area)) %>%
    dplyr::summarise(language_richness = sum(language_richness * area_prop),
                     language_evenness = sum(language_evenness * area_prop),
                     languages_threatened = sum(languages_threatened * area_prop),
                     languages_prop_threatened = sum(languages_prop_threatened * area_prop),
                     total_L1_POP = sum(total_L1_POP * area_prop)#,
                     #species_endangered = sum(species_endangered * area_prop),
                     #species_prop_endangered = sum(species_prop_endangered * area_prop)
                     ),
  ...
  jager_name_legend = jager_names %>%
    ...
    dplyr::mutate(full_name = paste(wls_fam, wls_gen, names, sep = ".")) %>%
    dplyr::mutate(full_name = gsub("-", "_", full_name, fixed = TRUE)),
  
  # world_languages = readr::read_csv("data/world_languages/WorldLanguagebyCountry.240919.csv") %>%
  ...
  #   dplyr::left_join(base_map %>%
  #                      dplyr::select(territory = admin, iso3c_2 = sov_a3)) %>%
  #   dplyr::mutate(iso3c = ifelse(is.na(iso3c), iso3c_2, iso3c)) %>%
  #   dplyr::select(iso3c, world_language = `World Language`) %>%
  #   dplyr::mutate(world_language = strsplit(world_language, ", ")) %>%
  #   tidyr::unnest(cols = c(world_language)) %>%
  #   dplyr::distinct() %>%
  #   dplyr::arrange(iso3c, world_language) %>%
  #   dplyr::group_by(iso3c) %>%
  #   dplyr::summarise(world_language = paste(world_language, collapse = ", ")) %>%
  #   dplyr::mutate(world_language_as_official = ifelse(world_language == "n", 0, 1)) %>%
  #   na.omit(),
  ...
  world_language_iso = world_languages_initial %>%
    dplyr::filter(is.na(iso3c) | !iso3c %in% unique(base_map$iso_a3)) %>%
    dplyr::mutate(territory = ifelse(territory %in% names(problematic_territory_names),
                                     problematic_territory_names[territory], territory)) %>%
    dplyr::left_join(base_map %>%
                       dplyr::select(territory = admin, iso_a3 = iso_a3)) %>%
  ...
  world_languages = readr::read_csv(file_in("data/world_languages/WorldLanguagebyCountry.280420.csv")) %>%
    ...
    dplyr::mutate(iso3c = ifelse(is.na(iso3c), iso_a3, iso3c)) %>%
    dplyr::select(-iso_a3) %>%
    # dplyr::left_join(base_map %>%
    #                    dplyr::select(admin, iso3c = iso_a3)) %>%
    #dplyr::mutate(iso3c = ifelse(is.na(iso3c), iso3c_2, iso3c)) %>%
    dplyr::select(iso3c, world_language = `World Language`) %>%
    dplyr::mutate(world_language = strsplit(world_language, ", ")) %>%
    tidyr::unnest(cols = c(world_language)) %>%
    dplyr::distinct() %>%
    dplyr::arrange(iso3c, world_language) %>%
    dplyr::mutate(world_language = ifelse(world_language == "German", "n", world_language) %>%
                    as.factor()) %>%
    dplyr::mutate(world_language_as_official = ifelse(world_language == "n", 0, 1)) %>%
    dplyr::group_by(iso3c) %>%
    dplyr::summarise(world_language_as_official = world_language_as_official[1],
                     world_language = list(unique(world_language))) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(world_language = purrr::map(world_language,
                                                 ~{nam <- levels(.x)
                                                 bin <- tabulate(as.numeric(.x),
                                                                 nbins = nlevels(.x))
                                                 names(bin) <- nam
  ...
  EGIDS = glotto_main %>%
    dplyr::select(LANG_ISO,
                  EGIDS = Language_status_ethnologue) %>%
    dplyr::left_join(lang_polys_merged %>%
                       dplyr::as_tibble() %>%
                       dplyr::select(LANG_ISO, LMP_POP1)) %>%
    tidyr::separate(EGIDS, c("EGIDS", "rest"), " ") %>%
    dplyr::select(-rest),
  
  glotto_main = readr::read_tsv(file_in("data/tree_lgs_df_glottolog_matchup.tsv"),
                                col_types = cols(
                                  Name = col_character(),
                                  Name_stripped_no_spaces = col_character(),
                                  iso639_3 = col_character(),
                                  iso639_3_macro = col_character(),
                                  glottocode = col_character(),
                                  Family_name_glottolog = col_character(),
                                  Classification_ethnologue = col_character(),
                                  Population_numeric_ethnologue = col_double(),
                                  endangerment_status_glottolog = col_character(),
                                  desc_status_glottolog = col_character(),
                                  Language_status_ethnologue = col_character(),
                                  level = col_character(),
                                  AUTOTYP_area = col_character(),
  ...
  glotto_contact = readr::read_tsv(file_in("data/Contact_lgs_from_tree.tsv"),
                                   col_types = cols(
                                     Name = col_character(),
                                     Name_stripped_no_spaces = col_character(),
                                     iso639_3 = col_character(),
  ...
  LEI_data = read_csv(file_in("data/endangerment/LEI_database_file.csv"),
                      col_names = c("some_num_1", "LANG_ISO", "some_name_1",
                                    "some_name_2", "LEI", "some_num_2",
                                    "some_name_3", "some_name_4", "some_notes_1",
                                    "some_notes_2", "some_place_1", "some_place_2",
                                    "coords")) %>%
    tidyr::separate(LEI, c("LEI", "LEI_Certainty"), " \\(") %>%
    tidyr::unnest(LANG_ISO = strsplit(LANG_ISO, ",")) %>%
    #dplyr::mutate(LANG_ISO = gsub("\\-.*", "", LANG_ISO)) %>%
  ...
  lang_endangerment_scores = get_all_endangerment_scores(lang_ids, glotto_main, EGIDS, LEI_data, UNESCO_data),
  ...
  multi_ISO_summ_AES_EGIDS = lang_endangerment_scores_AES_EGIDS %>%
    dplyr::group_by(LANG_ISO) %>%
    dplyr::summarise(count = n(), num_AES = n_distinct(AES), num_EGIDS = n_distinct(EGIDS),
                     AES = list(AES), EGIDS = list(EGIDS), IDS = list(ID),
                     LMP_POP1 = list(LMP_POP1)),
  
  lang_endangerment_by_ISO_AES_EGIDS = lang_endangerment_scores_AES_EGIDS %>%
    dplyr::group_by(LANG_ISO) %>%
    dplyr::summarise(AES = min(AES, na.rm = TRUE), EGIDS = min(EGIDS, na.rm = TRUE),
                     L1_POP = mean(LMP_POP1, na.rm = TRUE)),
    
  
  lang_endangerment_by_ISO = lang_endangerment_scores %>%
    dplyr::group_by(LANG_ISO, AES, EGIDS, LEI, UNESCO) %>%
    dplyr::summarise(count = n()),
  
  multilanguage_summary = lang_endangerment_scores %>%
    dplyr::group_by(LANG_ISO) %>%
    dplyr::summarise(count = n(),
              LANG_ISOs = list(LANG_ISO),
              iso_macro = list(iso_macro),
              endangerment_glottolog = list(AES),
              endangerment_ethnologue = list(Language_status_ethnologue)) %>%
  ...
  all_multi_ISOs = lang_endangerment_scores %>%
    ...
    dplyr::filter(count > 1) %>%
    dplyr::arrange(dplyr::desc(count), LANG_ISO) %>%
  ...
  language_types = readr::read_tsv(file_in("data/hedvig_output/Contact_lgs.tsv")) %>%
    ...
    dplyr::select(LANG_ISO = ISO639P3code,
                  type)%>%
    add_macrolanguages(macro_languages) %>%
    dplyr::left_join(lang_endangerment_by_ISO_AES_EGIDS %>%
                       dplyr::left_join(wlms_dat %>%
                                          dplyr::select(LANG_ISO = ISO, IM)) %>%
                       dplyr::select(LANG_ISO, IM), .) %>%
    dplyr::left_join(official_languages,
                     by = "LANG_ISO") %>%
    dplyr::mutate(official_in_any_country = ifelse(is.na(official_in_any_country), 0, 1),
                  type = ifelse(is.na(type), "prevalent", type)) %>%
    dplyr::group_by(LANG_ISO) %>%
    dplyr::summarise(type = type[1], official_in_any_country = mean(official_in_any_country),
                     IM = ifelse("Y" %in% IM, "Y", "N")) %>%
    dplyr::mutate(type = as.factor(type)),
  
  #polygon_area_plots = make_polygon_area_plots(language_areas, hua_data_2000_language_diversity),
  ...
  hmr_reprojected_lang_polys = st_transform(lang_polys, hmr_proj4string) %>%
    st_cast("MULTIPOLYGON"),
  
  #human_footprint_dat = get_hmr_raster_values(file_in(human_footprint_rasters), hmr_reprojected_lang_polys),
  ...
  human_footprint_dat = target(get_hmr_raster_values(human_footprint_raster = x, scaling = y, hmr_reprojected_lang_polys, lang_polys),
                               transform = map(x = !!human_footprint_rasters, y = !!scaling,
                                               .id = x)),
  
  connectivity_raster_dat = target(get_hmr_raster_values(human_footprint_raster = x, scaling = y, lang_circles_hmr_projected, lang_circles),
                               transform = map(x = !!human_footprint_rasters, y = !!scaling,
                                               .id = x)),
  
  IUCN_data = make_IUCN_data(IUCN_by_grid, IUCN_all_by_grid, IUCN_grid_raster,
                             lang_polys_merged_cea),
  
  diffs = list(tibble(hfp_change = mapply(function(x, y) x - y / 
                                            (actual_years["hfp_2009"] - 
                        ...
                        lapply(as.vector)),
               tibble(built_change = mapply(function(x, y) x - y / 
                                              (actual_years["built_2009"] - 
                                                 actual_years["built_1993"]), 
                              human_footprint_dat_data.HumanFootprint.Maps.Built2009.tif,
                              human_footprint_dat_data.HumanFootprint.Maps.Built1994.tif) %>%
                        lapply(as.vector)),
               tibble(croplands_change = mapply(function(x, y) x - y / 
                                                  (actual_years["croplands_2009"] - 
                                                     actual_years["croplands_1993"]), 
                        ...
                        lapply(as.vector)),
               tibble(nav_water_change = mapply(function(x, y) x - y / 
                                                  (actual_years["navwater_2009"] - 
                                                     actual_years["navwater_1993"]), 
                        ...
                        lapply(as.vector)),
               tibble(pasture_change = mapply(function(x, y) x - y / 
                                                (actual_years["pasture_2009"] - 
                                                   actual_years["pasture_1993"]), 
                                human_footprint_dat_data.HumanFootprint.Maps.Pasture2009.tif,
                                human_footprint_dat_data.HumanFootprint.Maps.Pasture1993.tif) %>%
                        lapply(as.vector)),
               tibble(pop_density_change = mapply(function(x, y) x - y / 
                                                    (actual_years["popdens_2009"] - 
                                                       actual_years["popdens_1993"]), 
  ...
  diffs_nb = list(tibble(hfp_change = mapply(function(x, y) x - y / 
                                            (actual_years["hfp_2009"] - 
                                               actual_years["hfp_1993"]), 
  ...
  land_use = list(tibble(hfp = human_footprint_dat_data.HumanFootprint.Maps.HFP2009.tif %>%
                           lapply(as.vector)),
                  tibble(built = human_footprint_dat_data.HumanFootprint.Maps.Built2009.tif %>%
                           lapply(as.vector)),
                  tibble(croplands = human_footprint_dat_data.HumanFootprint.Maps.croplands2005.tif %>%
                           lapply(as.vector)),
                  # tibble(nav_water = human_footprint_dat_data.HumanFootprint.Maps.NavWater2009.tif %>%
                  #          lapply(as.vector)),
                  tibble(pasture = human_footprint_dat_data.HumanFootprint.Maps.Pasture2009.tif %>%
                           lapply(as.vector)),
                  tibble(pop_density = human_footprint_dat_data.HumanFootprint.Maps.Popdensity2010.tif %>%
                           lapply(as.vector))#,
                  # tibble(roads = human_footprint_dat_data.HumanFootprint.Maps.Roads.tif %>%
                  #          lapply(as.vector))
                  ) %>%
    dplyr::bind_cols(),
  
  tempseasonality_projections_df = list(tibble(ts_2060 = tempseasonality_projections_2060 %>%
                                                 lapply(function(x) as.vector(x[ , 4]))),
  ...
  land_use_nb = list(tibble(hfp = connectivity_raster_dat_data.HumanFootprint.Maps.HFP2009.tif %>%
                           lapply(as.vector)),
                  tibble(built = connectivity_raster_dat_data.HumanFootprint.Maps.Built2009.tif %>%
                           lapply(as.vector)),
                  tibble(croplands = connectivity_raster_dat_data.HumanFootprint.Maps.croplands2005.tif %>%
                           lapply(as.vector)),
                  # tibble(nav_water = connectivity_raster_dat_data.HumanFootprint.Maps.NavWater2009.tif %>%
                  #          lapply(as.vector)),
                  tibble(pasture = connectivity_raster_dat_data.HumanFootprint.Maps.Pasture2009.tif %>%
                           lapply(as.vector)),
                  tibble(pop_density = connectivity_raster_dat_data.HumanFootprint.Maps.Popdensity2010.tif %>%
                           lapply(as.vector))#,
                  # tibble(roads = connectivity_raster_dat_data.HumanFootprint.Maps.Roads.tif %>%
                  #          lapply(as.vector))
  ) %>%
    dplyr::bind_cols(),
  ...
  connectivity_nb = list(tibble(nav_water_nb = connectivity_raster_dat_data.HumanFootprint.Maps.NavWater2009.tif %>%
                            lapply(as.vector)),
                  tibble(pop_density_nb = connectivity_raster_dat_data.HumanFootprint.Maps.Popdensity2010.tif %>%
                           lapply(as.vector)),
                  tibble(roads_nb = connectivity_raster_dat_data.HumanFootprint.Maps.Roads.tif %>%
                            lapply(as.vector))
  ) %>%
    dplyr::bind_cols(),
  ...
  hua_reprojected_lang_polys = st_transform(lang_polys, hua_proj4string) %>%
    st_cast("MULTIPOLYGON"),
  ...
  hua_vars_tibble = lapply(hua_vars_dat, function(x) x %>%
                             as.data.frame() %>%
                             as.list() %>%
  ...
  hua_vars_nb_tibble = lapply(hua_vars_nb, function(x) x %>%
                             as.data.frame() %>%
                             as.list() %>%
                             lapply(function(x) list(x)) %>%
                             dplyr::as_tibble()) %>%
  ...
  # soceco_dat_v3 = readr::read_csv(file_in("data/SocEco/LangEndang_SoEcData_v3_updated.csv")) %>%
  #   dplyr::rename_with(~paste0(.x, "_v3"), 
  #                      .cols = GDP_2010USD_10yr_median:`Urban_pop_pc_change_1960-2017`),
  
  soceco_dat = readr::read_csv(file_in("data/SocEco/LangEndang_SoEcData_v4.csv")),
  
  # countries_w_soceco = base_map_cea %>%
  #   dplyr::left_join(soceco_dat %>%
  #                      dplyr::select(adm0_a3 = ADM0_A3,
  #                                    GDP_2010USD_10yr_median:`Urban_pop_pc_change_1960-2017`)) %>%
  #   dplyr::left_join(world_languages, by = c("adm0_a3" = "iso3c")) %>%
  #   dplyr::left_join(education_policy %>%
  #                      dplyr::select(ADM0_A3, minority_education_policy = Minority_Ed_Policy), 
  #                    by = c("adm0_a3" = "ADM0_A3")) %>%
  #   dplyr::rename(urban_change = `Urban_pop_pc_change_1960-2017`),
  
  countries_w_soceco = base_map_cea %>%
    dplyr::left_join(soceco_dat %>%
                       dplyr::select(iso_a3 = ISO_A3,
                                     GDP_2010USD_10yr_median,
                                       Gini_SWIID_10yr_median,
                                       Ed_spending_pcGDP_10yr_median,
                                       Ed_spending_yr,
                                       Mean_yr_school_10yr_median,
                                       Life_exp_birth_10yr_median,
                                       Life_exp_sixty_average,
                                       `Urban_pop_pc_change_1960-2017`,
                                       Minority_Ed_Policy,
                                       gdp.pcap.10yrmed,
                                       expense.pcGDP.10yrmed,
                                       ed.sp.prm.pcGDP.10yrmed,
                                       ed.sp.sec.pcGDP.10yrmed,
                                       ed.sp.tert.pcGDP.10yrmed,
                                       ed.sp.tot.pcxpd.10yrmed
) %>%
                       dplyr::mutate(iso_a3 = ifelse(iso_a3 == "SDS", "SSD", iso_a3)) %>%
                       dplyr::mutate(iso_a3 = ifelse(iso_a3 == "SAH", "ESH", iso_a3)) %>%
                       dplyr::mutate(iso_a3 = ifelse(iso_a3 == "KOS", "XKX", iso_a3))) %>%
    dplyr::left_join(world_languages, by = c("iso_a3" = "iso3c")) %>%
    dplyr::left_join(education_policy %>%
                       dplyr::select(ISO_A3, minority_education_policy = Minority_Ed_Policy), 
                     by = c("iso_a3" = "ISO_A3")) %>%
    dplyr::rename(urban_change = `Urban_pop_pc_change_1960-2017`),
  
  # language_dup_plots = make_language_dup_plots(multilanguage_summary, 
  #                                              lang_polys,
  #                                              base_map,
  #                                              file_out("figures/language_dup_plots.pdf"))
  lang_polys_w_soceco_ids_initial = st_intersection(lang_polys_cea, countries_w_soceco),
  
  unmerged_langs = lang_polys_cea %>%
    dplyr::filter(!LANG_ISO %in% lang_polys_w_soceco_ids_initial$LANG_ISO),
  
  merge_unmerged = unmerged_langs[1:nrow(unmerged_langs), ] %>%
    sf::st_join(countries_w_soceco, join = sf::st_nearest_feature),
  
  lang_polys_w_soceco_ids = lang_polys_w_soceco_ids_initial %>%
    rbind(merge_unmerged),
  
  # soceco_by_lang = lang_polys_w_soceco_ids %>%
  ...
  #   # ungroup() %>%
  #   # dplyr::group_by(LANG_ISO) %>%
  #   dplyr::summarise_at(dplyr::vars(GDP_2010USD_10yr_median,
  #                                   Gini_SWIID_10yr_median,
  #                                   Ed_spending_pcGDP_10yr_median,
  #                                   Mean_yr_school_10yr_median,
  #                                   Life_exp_birth_10yr_median,
  ...
  #                                   minority_education_policy),
  #                       ~ sum(. * prop_area, na.rm = TRUE)) %>%
  #   dplyr::left_join(lang_polys_w_soceco_ids %>%
  #                      dplyr::bind_cols(area = as.vector(sf::st_area(lang_polys_w_soceco_ids))) %>%
  #                      dplyr::as_tibble() %>%
  #                      dplyr::group_by(LANG_ISO) %>%
  #                      dplyr::mutate(prop_area = area / sum(area)) %>%
  #                      dplyr::summarise_at(dplyr::vars(world_language),
  #                                          ~ ifelse(any(na.omit(.) != "n"), 
  #                             .[which.max(prop_area)],
  #                             "n"))),
  ...
  world_language_averaged = soceco_areas %>%
    dplyr::ungroup() %>%
    dplyr::mutate(world_language = purrr::map2(world_language, prop_area,
                                              ~.x * .y)) %>%
    dplyr::group_by(LANG_ISO) %>%
    dplyr::group_modify(~ dplyr::tibble(world_language = list(do.call(rbind, .x$world_language) %>%
                                          apply(2, sum_w_NA)))), 
  ...
  soceco_by_lang = soceco_areas %>%
    ...
    dplyr::mutate(world_language_as_official = 
                    ifelse(sapply(world_language, length) == 0, NA, 
                           world_language_as_official)),
  ...
  world_language_sanity_check = soceco_by_lang %>%
    dplyr::select(LANG_ISO, world_language_as_official,
                  world_language) %>%
    tidyr::unnest_wider(world_language) %>%
    dplyr::left_join(lang_names) %>%
    dplyr::select(LANG_ISO, Name, dplyr::everything()) %>%
    dplyr::filter(LANG_ISO %in% final_data$LANG_ISO) %>%
    dplyr::arrange(!is.na(world_language_as_official), 
                   world_language_as_official) %>%
  ...
  grid_poly_data_poly = land_use %>%
    ...
    dplyr::bind_cols(lang_polys %>%
                     ...
                     .) %>%
    dplyr::group_by(LANG_ISO) %>%
    dplyr::summarise(dplyr::across(.fns = ~ list(do.call(c, as.list(.x)))),
                     .groups = "keep") %>%
    dplyr::summarise(dplyr::across(.fns = ~ purrr::map_dbl(., ~ mean(.x, na.rm = TRUE)))),

  # projections_data_poly = tempseasonality_projections_df %>%
  #   dplyr::bind_cols(lang_polys %>%
  #                      as_tibble %>%
  #                      dplyr::select(LANG_ISO),
  #                    .) %>%
  ...
  #                    .groups = "keep") %>%
  #   dplyr::summarise(dplyr::across(.fns = ~ purrr::map_dbl(., ~ mean(.x, na.rm = TRUE)))) %>%
  #   dplyr::left_join(final_data_w_info %>%
  #                      dplyr::select(LANG_ISO = id_ISO_lang), .),
  ...
  projections_data_poly_correct_csv = readr::write_csv(ts_projections_by_lang,
                                               file_out("data/final_data/tempseasonality_projections_final.csv")),
  ...
  grid_poly_data_nb = lang_circles %>%
    ...
    dplyr::mutate_at(dplyr::vars(-LANG_ISO), ~ purrr::map_dbl(., ~ mean(., na.rm = TRUE))) %>%
    dplyr::group_by(LANG_ISO) %>%
    dplyr::summarise_all(~ mean(., na.rm = TRUE)),
  ...
  grid_poly_data = grid_poly_data_poly %>%
    ...
    dplyr::bind_rows(grid_poly_data_poly %>%
                       dplyr::filter(is.finite(hfp)) %>%
                       dplyr::select(LANG_ISO:pop_density_change) %>%
                       dplyr::mutate(used_nb_for_landuse = 0)) %>%
    dplyr::left_join(grid_poly_data_poly %>%
                       dplyr::select(-hfp:-pop_density_change)),
  
  connectivity_nb_data = connectivity_nb %>%
    dplyr::bind_cols(hua_vars_nb_tibble) %>%
    dplyr::mutate_all(~ purrr::map_dbl(., ~ mean(., na.rm = TRUE))) %>%
    dplyr::bind_cols(lang_polys_unioned %>%
                       as_tibble %>%
                       dplyr::select(LANG_ISO),
                     .),
  
  used_in_ed = readr::read_csv(file_in("data/education/EducLangsFinal.txt"), col_names = c("LANG_ISO")) %>%
    dplyr::mutate(used_in_education = 1) %>%
    add_macrolanguages(macro_languages),
  
  data_meta = tibble::tribble(
            ~Variable.Description, ~Dataset.Column.Name,              ~Data.Type, ~Suggested.Transformation,  ~Map.Link, ~Distribution.Link,                                                                                                                             ~Comments,
              "Language ISO Code",           "LANG_ISO", "Unordered Categorical",                    "None",    "N / A",            "N / A", "This is the code used to identify what constitutes individual languages in our analysis. This code is technically called ISO-639-3.",
     "Language Endangerment: AES",                "AES",               "Ordinal",                    "None", "#AES_map",        "#AES_dist",                                                            "One of the two endangerment indexes that will be our response variables."
     ),
  
  language_areas_merged = sf::st_area(lang_polys_merged) %>%
    {as.vector(.) / 1000 / 1000} %>%
    dplyr::tibble(LANG_ISO = lang_polys_merged$LANG_ISO,
                  polygon_area = .),
  
  new_language_diversity = make_new_language_diversity(lang_circle_intersections,
                                                       lang_endangerment_by_ISO_AES_EGIDS),
  
  bring_it_in = bring_it_all_in(lang_endangerment_by_ISO_AES_EGIDS, soceco_by_lang, language_types,
  ...
  mapping_sf = lang_polys_merged %>%
    ...
    rbind(lang_polys_merged %>%
            dplyr::as_tibble() %>%
            dplyr::select(-geometry) %>%
            dplyr::left_join(lang_circles) %>%
            dplyr::left_join(island_langs, by = "LANG_ISO") %>%
            dplyr::filter(island == 1) %>%
            sf::st_as_sf()),
  
  variable_maps = target(make_variable_maps(lang_polys_merged_cea, 
                                     bring_it_in, base_map_unioned, 
                                     transformation_list, transformation_table,
                                     mapping_sf),
                         format = "rds"),
  
  map_w_world_test = make_variable_maps(lang_polys_merged_cea, 
                                        final_data_w_world %>%
                                          dplyr::select(LANG_ISO, EGIDS), base_map_unioned, 
                                        transformation_list, transformation_table,
                                        mapping_sf),
  
  map_wo_world_test = make_variable_maps(lang_polys_merged_cea, 
                                        final_data %>%
                                          dplyr::select(LANG_ISO, EGIDS), base_map_unioned, 
                                        transformation_list, transformation_table,
                                        mapping_sf),
  
  # variable_maps_interactive = target(make_variable_maps2(lang_polys_merged, 
  #                                                         final_data_w_info, 
  #                                                         base_map, 
  #                                                         use_3d = FALSE),
  #                                     format = "rds"),
  
  high_res_EGIDS_map = {variable_maps[["EGIDS"]] + theme(legend.key.size = unit(0.05, "npc"),
                                                        legend.text = element_text(size = 28),
                                                        legend.title = element_text(size = 42))} %>%
    ggsave(file_out("figures/big_EGIDS_map.pdf"), ., width = 40, height = 25),

  miss_vis_1 = naniar::vis_miss(bring_it_in) +
    theme(plot.margin = unit(c(0.1, 2, 0.1, 0.1), "inches")),

  all_data_transformed = map2_dfc(bring_it_in, 
                                  transformation_list[colnames(bring_it_in)],
                                  ~if(is.numeric(.x)) {.y$transform(.x)} else {.x}), # %>%
    #magrittr::set_colnames(paste(colnames(bring_it_in), "_tr")),

  corr_plots = make_corr_plots(all_data_transformed),
  
  corr_plot_final = make_corr_plots(final_data_w_info %>%
                                      dplyr::mutate(response_EGIDS_lang =
                                                      as.numeric(response_EGIDS_lang)) %>%
                                      dplyr::select(response_EGIDS_lang,
                                                    lang_island_lang,
                                                    dplyr::ends_with("_tr"))),
  ...
  corr_final = round(cor(final_data_w_info %>%
                           dplyr::mutate(response_EGIDS_lang =
                                           as.numeric(response_EGIDS_lang)) %>%
                           dplyr::select(response_EGIDS_lang,
                                         lang_island_lang,
  ...
  corr_final_csv = dplyr::as_tibble(corr_final,
                                    rownames = "variable") %>%
    readr::write_csv(file_out("data/final_variables_corr_matrix.csv")),
  
  transformed = transformation_table$column_name[transformation_table$transformation_name != "identity"] %>%
    match(colnames(all_data_transformed)),
  
  endangered_outlier_plot = make_endangered_outlier_plot(bring_it_in),
  
  # islands = sf::st_read("data/islands/final_bigislandsnamed") %>%
  #   sf::st_set_crs(54009) %>%
  #   dplyr::mutate(id = 1:dplyr::n()),
  
  #lang_overlap_islands,
  
  final_data_all = bring_it_in %>%
    dplyr::left_join(all_data_transformed %>%
                       dplyr::select(-AES, -EGIDS),
                     by = "LANG_ISO",
                     suffix = c("", "_tr")),
  ...
  final_data = final_data_all %>%
    ...
    dplyr::select(-nav_water_change, -nav_water_change_tr, -popden, -popden_tr,
                  -category, -category_tr, -altitude, -altitude_tr,
                  -annual_precipitation, -annual_precipitation_tr,
                  -mgs, -mgs_tr) %>%
    #dplyr::filter(!LANG_ISO %in% unknown_loc$LANG_ISO) %>%
    tidyr::drop_na(EGIDS),
  ...
  tree_ethno = ape::read.tree(file_in("data/trees/ethno_tree.tre")) %>%
    ape::drop.tip(which(!.$tip.label %in% final_data$LANG_ISO)),
  ...
  glottolog_max_class = glottolog_extra %>%
    dplyr::mutate(class_split = strsplit(classification, "/", fixed = TRUE),
                  n_class = sapply(class_split, length)) %>%
    dplyr::summarise_at(vars(n_class), ~max(., na.rm = TRUE)),
  
  glottolog_class = glottolog_extra %>%
    tidyr::separate(classification, paste0("level_", 1:glottolog_max_class$n_class), "/") %>%
    dplyr::select(dplyr::starts_with("level"), level_18 = LANG_ISO, -level),
  
  class_df = glottolog_class %>%
    as.data.frame() %>%
    apply(1, function(x) {x[is.na(x)] <- x["level_18"]; x}) %>%
    t() %>%
  ...
  #phy_corr_glottolog = vcv.phylo(tree_glottolog, corr = TRUE),
  
  lang_polys_merged_simplified = lang_polys_merged %>%
    rmapshaper::ms_simplify(0.0005, keep_shapes = TRUE),
  
  spat_dists = target(sf::st_distance(lang_polys_merged_simplified),
                      trigger = trigger(condition = fix_5_errors != 1,
                                        mode = "blacklist")),
  
  spat_dist_mat = as.matrix(spat_dists) / 1000, ## convert to km
  ...
  # unknown_loc = final_data_all %>%
  #   dplyr::left_join(lang_polys %>%
  #                      dplyr::as_tibble() %>%
  #                      dplyr::select(LANG_ISO, LMP_CLASS)) %>%
  #   dplyr::group_by(LANG_ISO) %>%
  #   dplyr::summarise(count = n(), LMP_CLASS = list(as.character(LMP_CLASS))) %>%
  #   dplyr::filter(count == 1) %>%
  #   dplyr::filter(sapply(LMP_CLASS, function(x) x == "U")),
  
  double_check_maps = make_double_check_maps(lang_polys_merged_cea, bring_it_in, base_map, 
  ...
  # EGIDS_map_labelled = make_EGIDS_map_labelled(lang_polys_merged_cea, bring_it_in, base_map_unioned) %>%
  #   ggsave(file_out("figures/big_EGIDS_map_labelled.pdf"), ., width = 45, height = 28.125, limitsize = FALSE),
  
  EGIDS_map_final = make_EGIDS_map_final(lang_polys_merged_cea, final_data, base_map_unioned) %>%
    ggsave(file_out("figures/big_EGIDS_map_final.pdf"), ., width = 45, height = 28.125, limitsize = FALSE),
  ...
  final_data_polys_w_data_cea_shp_file = sf::st_write(final_data_polys_w_data_cea,
                                                      file_out("data/final_polys/final_data_polys_w_data_wgs84.geojson"),
                                                      append = FALSE),
  ...
  check_final_data_regions = final_data_w_info %>%
    dplyr::left_join(final_data_polys_cea %>% dplyr::as_tibble() %>% dplyr::select(LANG_ISO, CNT) %>%
                       dplyr::group_by(LANG_ISO) %>%
                       dplyr::summarise(CNT = CNT[1]),
                     by = c("id_ISO_lang" = "LANG_ISO")) %>%
    dplyr::filter((lang_continent_lang == "Europe" & CNT != "Europe") |
                  (lang_continent_lang == "Asia" & CNT != "Asia") |
                    (lang_continent_lang == "North America" & CNT != "Americas") |
                    (lang_continent_lang == "South America" & CNT != "Americas")) %>%
    dplyr::select(id_ISO_lang:lang_island_lang, CNT),
  ...
  latent_preds = purrr::map(list.files("data/latent", full.names = TRUE),
                            ~readr::read_csv(.x)) %>%
    purrr::set_names(list.files("data/latent")),
  
  latent_preds2 = purrr::map(list.files("data/latent2_w_continent/", full.names = TRUE),
                            ~readr::read_csv(.x)) %>%
  ...
  base_map_exploded_cea = base_map_cea %>%
    ...
    sf::st_sf() %>%
    sf::st_make_valid() %>%
    dplyr::mutate(landmass_id = paste0("landmass_", 1:n())),
  ...
  island_langs = island_assignment %>%
    dplyr::mutate(island = ifelse(max_area < 10000, 1, 0)) %>%
    dplyr::as_tibble() %>%
    dplyr::select(LANG_ISO, island),
  ...
  final_data_w_info = final_data %>%
    ...
    dplyr::left_join(glotto_main %>%
                       dplyr::select(LANG_ISO,
                                     Name) %>%
                       dplyr::group_by(LANG_ISO) %>%
                       dplyr::summarise(names = list(unique(Name)),
                                        n = length(unique(Name))) %>%
                       dplyr::mutate(name = purrr::map_chr(names,
                                                           ~paste(.x, collapse = ", "))) %>%
                       dplyr::mutate(name = ifelse(n > 1,
                                                   paste0("Macrolanguage containing: ", name),
                                                   name)) %>%
                       dplyr::select(-n, -names)) %>%
    dplyr::left_join(continents) %>%
    dplyr::select(name, LANG_ISO, dplyr::everything()) %>%
    dplyr::arrange(LANG_ISO) %>%
    dplyr::select(  id_ISO_lang = LANG_ISO,
                    id_name_lang = name,
                    ...
                    soceco_gdp.pcap.10yrmed_country = gdp.pcap.10yrmed,
                    soceco_expense.pcGDP.10yrmed_country = expense.pcGDP.10yrmed,
                    ...
                    lang_L1.POP_lang_tr = L1_POP_tr,
                    soceco_Gini.SWIID.10yr.median_country_tr = Gini_SWIID_10yr_median_tr,
                    ...
                    landuse_urban.change_country_tr = urban_change_tr,
                    worldlang_world.language.as.official_country_tr = world_language_as_official_tr,
                    ...
                    lang_type_lang_tr = type_tr,
                    lang_official.in.any.country_lang_tr = official_in_any_country_tr,
                    landuse_hfp_polygon_tr = hfp_tr,
                    landuse_built_polygon_tr = built_tr,
                    landuse_croplands_polygon_tr = croplands_tr,
                    landuse_pasture_polygon_tr = pasture_tr,
                    landuse_pop.density_polygon_tr = pop_density_tr,
                    shift_hfp.change_polygon_tr = hfp_change_tr,
                    shift_built.change_polygon_tr = built_change_tr,
                    shift_croplands.change_polygon_tr = croplands_change_tr,
                    shift_pasture.change_polygon_tr = pasture_change_tr,
                    shift_pop.density.change_polygon_tr = pop_density_change_tr,
                    enviro_mgs_polygon_tr = mgs_new_tr,
                    enviro_temperature.seasonality_polygon_tr = temperature_seasonality_tr,
                    enviro_precipitation.seasonality_polygon_tr = precipitation_seasonality_tr,
                    div_language.richness_nb_tr = language_richness_tr,
                    div_threatened.languages_nb_tr = threatened_languages_tr,
                    div_prop.threatened.languages_nb_tr = prop_threatened_languages_tr,
                    div_language.evenness_nb_tr = language_evenness_tr,
                    conn_nav.water_nb_tr = nav_water_nb_tr,
                    landuse_pop.density_nb_tr = pop_density_nb_tr,
                    conn_roads_nb_tr = roads_nb_tr,
                    conn_altitude.range_tr = altitude_range_tr,
                    conn_roughness_nb_tr = roughness_nb_tr,
                    conn_river_nb_tr = river_nb_tr,
                    div_bordering.language.richness_lang_tr = bordering_language_richness_tr,
                    ...
                    lang_polygon.area_lang_tr = polygon_area_tr,
                    lang_documentation_lang_tr = documentation_tr) %>%
    ## fix Fillipino incorrect island classification
    dplyr::mutate(lang_island_lang = ifelse(id_ISO_lang == "fil",
                                            0,
                                            lang_island_lang)),

  final_data_w_projections = final_data_w_info %>%
    dplyr::left_join(ts_projections_by_lang %>%
                       dplyr::select(id_ISO_lang = LANG_ISO,
                                     proj_temperature.seasonality.2060_polygon =
                                       ts_2060,
                                     proj_temperature.seasonality.2100_polygon =
                                       ts_2100)),
  
  final_L1_POP = final_data %>%
    dplyr::select(LANG_ISO, L1_POP),
  
  final_L1_POP_csv = final_L1_POP %>%
    readr::write_csv(file_out("data/final_L1_POP.csv")),
  
  map_hex_robin = base_map_unioned %>%
    sf::st_transform('ESRI:54030') %>%
    sf::st_make_grid(n = c(154, 72), square = FALSE),

  hex_area = sf::st_area(map_hex_robin),
  
  # interactive_vis_1 = build_interactive_vis(human_footprint_rasters,
  #                                           base_map,
  #                                           lang_polys_merged),
  # 
  # interactive_vis_final = build_interactive_vis(final_data,
  #                                           base_map,
  #                                           lang_polys_unioned),
  ...
  regions_map = make_regions_map(base_map_w_antarctica,
                                 final_data_polys_w_data_cea),
  
  var_list = readr::read_csv(file_in("data/final_data/lang_endang_variable_list.csv")),
  
  var_tab = make_variable_table(final_data_w_info, transformation_table, var_list),
  ...
  voronoi_ids = readr::write_csv(lang_polys_voronoi %>%
                                   as.data.frame() %>%
                                   dplyr::select(LANG_ISO) %>%
                                   dplyr::semi_join(final_data_w_info,
                                                    by = c("LANG_ISO" = "id_ISO_lang")),
                                 file_out("data/list_of_voronoi_languages.csv"))
  
)

# ggplot(countries_w_soceco) +
#   geom_sf(aes(fill = GDP_2010USD_10yr_median)) +
...
#   theme_minimal()
# 
# ggplot(lang_polys_w_soceco_ids) +
#   geom_sf(aes(fill = GDP_2010USD_10yr_median)) +
#   scale_fill_continuous(trans = "log") +
#   theme_minimal()
# 
# ggplot(lang_polys_merged_cea %>% dplyr::left_join(soceco_by_lang)) +
#   geom_sf(aes(fill = Life_exp_birth_10yr_median), colour = NA, alpha = 0.5) +
#   #scale_fill_continuous(trans = "log") +
#   theme_minimal()
# 
# test <- lang_polys_w_soceco_ids %>%
#   as_tibble() %>%
#   dplyr::group_by(LANG_ISO) %>%
#   dplyr::summarise(count = n())

# ggplot(lang_polys_merged_simplified) +
#   geom_sf() +
#   theme_minimal()

# ggplot(diversity_by_lang) +
#   geom_sf(aes(fill = `language_richness`), colour = NA) +
#   scale_fill_continuous(trans = "log") +
#   theme_minimal()
# 
# ggplot(diversity_by_lang) +
#   geom_sf(aes(fill = `species_endangered`), colour = NA) +
#   scale_fill_continuous(trans = "log") +
#   theme_minimal()
# 
# ggplot(lang_polys_merged_cea %>% dplyr::left_join(language_grid_based_data_by_ISO)) +
#   geom_sf(aes(fill = species_endangered), colour = NA) +
#   scale_fill_continuous(trans = "log") +
#   theme_minimal()
# 
...
#   scale_fill_continuous(trans = "log") +
#   theme_minimal()
# ggplot(lang_pts %>% sf::st_transform(54030)) +
#   geom_sf(data = base_map %>%
#             sf::st_transform(54030), fill = "grey20", colour = NA) +
#   geom_sf(aes(colour = LMP_CLASS, fill = LMP_CLASS), alpha = 0.8, size = 0.4) +
#   scale_fill_discrete(labels = c("A - No polygon, only point",
#                                "L - Centroid of language with a polygon",
#                                "U - No polygon, unknown location",
#                                "W - No polygon, widespread language",
#                                "X - No polygon, extinct language",
#                                "NA - No polygon, unclassified")) +
#   scale_colour_discrete(labels = c("A - No polygon, only point",
#                                  "L - Centroid of language with a polygon",
#                                  "U - No polygon, unknown location",
#                                  "W - No polygon, widespread language",
#                                  "X - No polygon, extinct language",
#                                  "NA - No polygon, unclassified")) +
#   theme_minimal()
# ggplot(lang_polys_merged_cea) +
#   geom_sf() +
#   geom_sf(data = lang_polys_merged_cea %>% dplyr::filter(LANG_ISO == "cmn"), fill = "red", alpha = 0.6) +
#   theme_minimal()
# 
# 
# ggplot(lang_pts_voronoi %>% sf::st_transform(54030)) +
#   geom_sf(data = base_map %>%
#             sf::st_transform(54030), fill = "grey20", colour = NA) +
#   geom_sf(aes(fill = LMP_CLASS), alpha = 0.5, colour = NA) +
#   geom_sf(aes(colour = LMP_CLASS), fill = NA, size = 0.15) +
#   theme_minimal()

#piggyback::pb_track("data/lang/*")

# info<-r_drake_graph_info()
# r_vis_drake_graph(subset = info$nodes$id[info$nodes$type != "function"], selfcontained = TRUE,
#                   file = "language_endangerment_dependency_graph_of_objects.html", font_size = 8, hover = TRUE)
